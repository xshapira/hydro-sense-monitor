...
describe("UnitsHealthDashboard Component", () => {
  ...
  it("handles unit click", async () => {
    ...
    render(<UnitsHealthDashboard isVisible={true} onUnitClick={mockOnUnitClick} />)
  ...
  it("shows empty state when no units", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No units found. Start by submitting sensor readings."),
      ).toBeInTheDocument()
    })
  })
  ...
  it("applies correct styling based on health status", async () => {
    ...
    expect(unit1Button).toHaveClass("bg-green-100", "border-green-300", "text-green-900")
    expect(unit2Button).toHaveClass("bg-yellow-100", "border-yellow-300", "text-yellow-900")
    expect(unit3Button).toHaveClass("bg-red-100", "border-red-300", "text-red-900")
  })
})
