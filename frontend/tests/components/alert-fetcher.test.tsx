...
describe("AlertFetcher Component", () => {
  ...
  it("fetches and displays alerts successfully", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Recent Alerts for unit-123")).toBeInTheDocument()
    })
  ...
  it("shows green message when unit exists but has no alerts", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('No alerts found for unit "unit-456". This unit has no issues.'),
      ).toBeInTheDocument()
    })
  ...
  it("shows red message when unit doesn't exist", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('No alerts found for unit "unit-nonexistent". This unit doesn\'t exist.'),
      ).toBeInTheDocument()
    })
