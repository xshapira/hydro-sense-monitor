...
describe("Home Page Integration", () => {
	...
	it("renders main components", () => {
		...
		// Check buttons
		expect(
			screen.getByRole("button", { name: "View All Units" }),
		).toBeInTheDocument();
		expect(
	...
	it("prefills unit ID in alert fetcher when unit is clicked in dashboard", async () => {
		...
		await waitFor(() => {
			expect(
				screen.queryByText("All Units Health Status"),
			).not.toBeInTheDocument();
		});

		// Check unit ID is prefilled in alert fetcher
		const unitInput = screen.getByPlaceholderText(
			"Unit ID",
		) as HTMLInputElement;
	...
	it("shows random reading generator when dev tools are enabled", () => {
		...
		expect(
			screen.getByRole("button", { name: "Generate Random Reading" }),
		).toBeInTheDocument();
	});
	...
	it("fetches alerts when user enters unit ID and clicks Fetch Alerts", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("Recent Alerts for unit-456"),
			).toBeInTheDocument();
